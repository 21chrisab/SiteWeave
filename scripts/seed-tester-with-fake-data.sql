...
BEGIN
    ...
    VALUES (v_org_id, 'OrganizationAdmin', '{"can_create_tasks":true,"can_view_financials":true,"can_manage_users":true,"can_delete_projects":true,"can_assign_tasks":true,"can_manage_contacts":true,"can_create_projects":true,"can_edit_projects":true}'::jsonb, true, now())
    ON CONFLICT (organization_id, name) DO UPDATE SET updated_at = now()
    RETURNING id INTO v_role_id;
    IF v_role_id IS NULL THEN
        SELECT id INTO v_role_id FROM roles WHERE organization_id = v_org_id AND name = 'OrganizationAdmin';
    END IF;
